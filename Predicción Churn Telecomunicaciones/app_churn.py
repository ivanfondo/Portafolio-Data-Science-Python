...
def load_assets():
    try:
        assets = joblib.load('Predicción Churn Telecomunicaciones/modelo_churn_final.pkl')
        model = assets['model']
        features = assets['features'] 
        df_modelo = pd.read_csv('Predicción Churn Telecomunicaciones/datos_test_dashboard.csv', index_col=0) 
        df_visual = pd.read_csv('Predicción Churn Telecomunicaciones/TelcoChurn.csv', index_col=0)
...
if model is not None:
    ...
    # Filtro simplificado sin umbral manual (usamos 0.5 interno para categorías)
    ...
    if id_cliente:
        ...
        # KPIs con lógica de color automática (Rojo si > 60%)
